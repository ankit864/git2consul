...
var file_modified = function(branch, file, cb) {
  ...
  var handle_json_kv_file = function(file_path,common_properties_relative_path, kvs, cb) {
  ...
  fs.readFile(file_path, {encoding: 'utf8'}, function (err, file_body) {
    ...
    if(common_properties_relative_path) {
      ...
      var path_to_common_properties_file = global.gproperties;
      fs.readFile(path_to_common_properties_file, {encoding: 'utf8'}, function (err, common_body) {
        if (err) {
          logger.warn('Failed to read common variables for global file' + path_to_common_properties_file + ' due to ' + err);
          common_body = '';
        }
        logger.warn("path_to_common_properties_file global file success " + path_to_common_properties_file);
        logger.warn("path_to_common_properties_file global file success " + global.gproperties);
      ...
      var path_to_common_properties_file = global.gproperties;
      logger.warn("path_to_common_properties_file global file " + path_to_common_properties_file);
      logger.warn("path_to_common_properties_file global file success " + global.gproperties);
  ...
  var handle_expanded_keys_with_different_file_types = function(kvs) {
    if (file.endsWith('.json')) {
      handle_json_kv_file(fqf, "common", kvs, cb);
    } else if (file.endsWith('.yaml') || file.endsWith('.yml')) {
      handle_yaml_kv_file(fqf, kvs, cb);
    } else if (file.endsWith('.properties')) {
      handle_properties_kv_file(fqf, "common" , kvs, cb);
    } else {
